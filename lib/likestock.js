...
module.exports = {
    ...
    createLikeStock : function(db,  request, response){
        ...
        var query = mybatisMapper.getStatement('LikeStockMapper', 'createLikeStock', param, format);
    ...
    deleteLikeStock : function(db,  request, response){
        ...
        //질의문 형식
        var format = {language: 'sql', indent: '  '};
        var query = mybatisMapper.getStatement('LikeStockMapper', 'deleteLikeStock', param, format);
        //첫번째는 xml의 name값, 두번째는 해당 xml의 id값, 세번째는 파라미터, 마지막은 포맷이다.
